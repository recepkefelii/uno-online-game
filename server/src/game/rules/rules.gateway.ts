...
export class RulesGateway {
  ...
  async getMainCards(@ConnectedSocket() socket: any, emit: boolean) {
    try {
      const [gameId, uniqueEmit] = await this.getGameIdAndUsername(socket);
      const mainCard = await this.rulesService.getMainCard(gameId)
      if (!emit) {
        this.server.emit(uniqueEmit, mainCard)
      }
      return mainCard
    } catch (error) {
      throw new WsException(error.message);
  ...
  async getCard(@ConnectedSocket() socket: any) {
    const [gameId, username, uniqueEmit] = await this.getGameIdAndUsername(socket)
    await this.rulesService.getNewCard(username, gameId)
    const newCreateCard = await this.getCards(socket, true)
